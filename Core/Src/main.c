...
/*
void readReg( uint8_t id_addr,uint8_t reg)
{
	// address?�� ?��?��?�� i2c 구별?�� (tfluna?�� 0x10)
	// ?��?�� 발생?�� false 반환
	HAL_I2C_Master_Transmit(&hi2c1, tfluna, &reg, 1, 1000);
	HAL_I2C_Master_Receive(&hi2c1, tfluna, &received, 1, 1000);

}
void READ_tfluna_Data(void)
{
	for (uint8_t reg= 0x00; reg<2; reg++)
	{
		readReg(0x10,reg);
		dataArray[reg] = received;
	}
